...
def manage_hotspot(action='start'):
    ...
    if action == 'start':
        ...
        try:
            result = subprocess.run(['nmcli', '-t', '-f', 'DEVICE,TYPE', 'device'], stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True, check=True)
            wifi_device = None
        ...
        try:
            subprocess.run(['nmcli', 'con', 'down', HOTSPOT_SSID], stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True)
            subprocess.run(['nmcli', 'con', 'delete', HOTSPOT_SSID], stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True)
            subprocess.run(command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True, check=True, timeout=15)
            ...
            log_message("[!] 错误: 创建热点时出错: {}".format(e.stderr.strip()))
            return False
        ...
        try:
            subprocess.run(['nmcli', 'con', 'down', HOTSPOT_SSID], stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True, timeout=10)
            subprocess.run(['nmcli', 'con', 'delete', HOTSPOT_SSID], stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True, timeout=10)
    ...
    if PSUTIL_AVAILABLE:
        ...
        if wifi_interfaces:
            ...
            for iface in wifi_interfaces:
                try:
                    network = ipaddress.IPv4Network("{}/{}".format(iface['ip'], iface['netmask']), strict=False)
...
class RenamerThread(threading.Thread):
    ...
    def run(self):
        try:
            while not self.stop_event.is_set() and not EXIT_FLAG.is_set():
                new_name = "{}.{}{}".format(self.original_name_part, time.time_ns(), self.original_ext_part)
    ...
    def restore_filename(self):
        if self.current_path != self.original_path:
            ...
            except Exception as e: log_message("[!] 恢复文件名 '{}' 失败: {}".format(self.original_basename, e))
...
class StatusDisplayThread(threading.Thread):
    ...
    def run(self):
        ...
        while not self.stop_event.is_set():
            ...
            except (ValueError, TypeError): print("进度: {} ({})".format(progress_str, details))
            print("\n" + "--- 最近日志 ".ljust(60, "-"))
...
class FileScannerThread(threading.Thread):
    ...
    def __init__(self, task_queue, source_dirs, files_to_exclude):
        super(FileScannerThread, self).__init__(); self.task_queue = task_queue; self.source_dirs = source_dirs
        self.files_to_exclude = files_to_exclude; self.name = "文件扫描器"; self.daemon = True
    def run(self):
        log_message("[*] {} 已启动。".format(self.name))
        while not EXIT_FLAG.is_set():
            with STATUS_LOCK: WORKER_STATUS[self.name] = {"file": "N/A", "mode": "扫描目录", "progress": "---", "details": "扫描 {} 个源...".format(len(self.source_dirs))}
            ...
            if not EXIT_FLAG.is_set(): log_message("[*] {} 完成一轮扫描。".format(self.name)); time.sleep(random.uniform(MIN_FILE_DELAY_S, MAX_FILE_DELAY_S))
    ...
    def process_file(self, file_path):
        ...
        try:
            ...
            while not EXIT_FLAG.is_set():
                ...
                with STATUS_LOCK: WORKER_STATUS[self.name] = {"file": os.path.basename(file_path), "mode": "读取并入队", "progress": "{:.2f}%".format(progress), "details": "{:.2f} / {:.2f} MB".format(sent_mb, total_mb)}
        ...
        except (IOError, OSError) as e: log_message("[!] I/O 错误 '{}': {}".format(file_path, e))
        except Exception as e:
            if not EXIT_FLAG.is_set(): log_message("[!] 意外错误 '{}': {}".format(file_path, e))
        finally:
...
class ChunkSenderWorker(threading.Thread):
    ...
    def __init__(self, task_queue, name, stop_event, interface_infos):
        super(ChunkSenderWorker, self).__init__(); self.task_queue = task_queue; self.name = name
    ...
    def setup_sockets(self):
        for info in self.interface_infos:
            ...
            except Exception as e: log_message("[!] [{}] 创建套接字时出错: {}".format(self.name, e))
    def run(self):
        global bytes_sent_in_period
        self.setup_sockets()
        if not self.sockets: log_message("[!] [{}] 无可用套接字，线程退出。".format(self.name)); return
        try:
            while not EXIT_FLAG.is_set() and not self.stop_event.is_set():
                ...
                for sock, broadcast_addr in self.sockets:
                    try:
                        ...
                        if not EXIT_FLAG.is_set(): log_message("[!] [{}] 广播到 {} 失败: {}".format(self.name, broadcast_addr, e))
...
class WorkerManagerThread(threading.Thread):
    ...
    def __init__(self, task_queue, interface_infos):
        super(WorkerManagerThread, self).__init__(); self.name = "工人管理器"; self.daemon = True
        ...
        while not EXIT_FLAG.is_set():
            ...
            if current_count < target_count:
                for _ in range(target_count - current_count):
                    worker_id_counter += 1; worker_name = "发送器-{}".format(worker_id_counter); stop_event = threading.Event()
...
def graceful_shutdown(signum, frame):
    ...
    if alive: print("[*] 等待 {} 个工作线程完成...".format(len(alive)));
    ...
    try:
        ...
        for root, _, files in os.walk(directory):
            ...
            for filename in files:
                ...
                if match:
                    original_name = "{}{}".format(match.group(1), match.group(3)); current_path, restored_path = os.path.join(root, filename), os.path.join(root, original_name)
                    try:
                        if os.path.exists(restored_path): os.remove(current_path)
                        else: os.rename(current_path, restored_path)
                        restored_count += 1
                    except Exception as e: log_message("[!] [{}] 恢复 '{}' 失败: {}".format(thread_name, filename, e))
        if restored_count > 0: log_message("[*] [{}] 在 '{}' 中恢复了 {} 个中断的文件。".format(thread_name, directory, restored_count))
    except Exception as e: log_message("[!] [{}] 检查中断文件时出错: {}".format(thread_name, e))
    ...
    try:
        ...
        if not EXIT_FLAG.is_set():
            try:
                ...
                print("\n[!] Script encountered fatal error: {}".format(e))
            import traceback
            traceback.print_exc()
            EXIT_FLAG.set()