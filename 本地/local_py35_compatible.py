...
def safe_print(text):
    ...
    try:
        ...
        try:
            ...
            # 最后的降级：显示英文提示
            print("[Chinese characters - encoding issue]")
...
class StatusDisplayThread(threading.Thread):
    ...
    def run(self):
        ...
        while not self.stop_event.is_set():
            ...
            os.system(clear_cmd)
            safe_print("=" * 60)
            safe_print(" 数据蒲公英脚本 (模式: 局域网全速) ".center(60))
            safe_print("=" * 60)
            ...
            safe_print("网络模式: {:<20} | 当前速度: {:>7.2f} MB/s".format(hotspot_status, speed_mbps))
            safe_print("任务队列: {:<5} / {:<5} | 已完成文件: {} 个".format(TASK_QUEUE.qsize(), TASK_QUEUE.maxsize, FILE_SEND_COUNT))
            safe_print("会话流量: {:.4f} GB (本次运行)".format(session_gb))
            ...
            safe_print("当前文件: {}".format(file_name))
            try:
                progress_val = float(progress_str.strip('%'))
                bar_length = 40
                filled_length = int(bar_length * progress_val / 100)
                bar = '█' * filled_length + '-' * (bar_length - filled_length)
                safe_print("进度: |{}| {:>6.2f}% ({})".format(bar, progress_val, details))
            except (ValueError, TypeError): 
                safe_print("进度: {} ({})".format(progress_str, details))
            safe_print("\n" + "--- 最近日志 ".ljust(60, "-"))
...
def graceful_shutdown(signum, frame):
    ...
    if alive: 
        safe_print("[*] 等待 {} 个工作线程完成...".format(len(alive)))
    for thread in alive: 
        thread.join(timeout=5.0)
    safe_print("[*] 清理完成，程序即将退出。")
    ...
    try:
        if ENABLE_HOTSPOT_MODE:
            if not manage_hotspot(action='start'):
                safe_print("[!] 致命错误: 无法启动热点模式。请检查权限和NetworkManager状态。程序退出。")
                return

        interface_infos, connection_type = get_broadcast_addresses()
        if not interface_infos:
            safe_print("[!] 致命错误: 无法找到任何可用的网络接口进行广播。程序退出。")
            return
